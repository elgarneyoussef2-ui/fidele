  ...
  return (
    <Card className="border-accent/20 bg-accent/5 shadow-card">
      <CardHeader className="pb-3">
        <CardTitle className="text-sm eyebrow flex items-center gap-2">
          <Bell className="h-4 w-4 text-accent" />
          Demandes de récompenses
          <Badge className="bg-accent text-accent-foreground ml-1 h-5 px-1.5">{requests.length}</Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {requests.map(req => (
          <div key={req.id} className="bg-card rounded-lg p-3 flex items-center justify-between gap-3 shadow-sm border border-accent/10">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground num-mono">{req.reward_points} pts · {timeAgo(req.created_at)}</p>
              </div>
            </div>
            <div className="flex gap-1.5 shrink-0">
              ...
              <Button
                size="sm"
                variant="outline"
                className="h-8 px-3 gap-1 text-destructive border-destructive/20 hover:bg-destructive/10"
