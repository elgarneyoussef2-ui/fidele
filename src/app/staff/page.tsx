...
import {
  QrCode, RefreshCw, CheckCircle2, XCircle, Clock,
  Download, Printer, Loader2, LogOut, ShieldCheck,
  User, Lock, Plus, Bell,
} from 'lucide-react'
...
function LoginScreen({ onLogin }: { onLogin: (s: Staff) => void }) {
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', background: '#fff' }}>
      ...
      <p style={{ fontSize: 13, color: '#9CA3AF', marginBottom: 40, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Espace serveur</p>
      ...
      {!qrDataUrl ? (
          ...
          {amountNum > 0 && (
            <div style={{ marginTop: 12, background: '#EDE6FB', borderRadius: 14, padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} className="fadein">
              <div>
                ...
                <p style={{ fontSize: 11, color: '#7C3AED', opacity: 0.65, marginTop: 2 }}>1 pt = {madPerPoint} MAD</p>
              </div>
        ...
        <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }} className="popin">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#16A34A', background: '#F0FDF4', border: '1px solid #BBF7D0', padding: '7px 14px', borderRadius: 99, fontWeight: 600 }}>
            <ShieldCheck size={13} /> À usage unique
          </div>
          ...
          <button
            onClick={reset}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 14, color: '#9CA3AF', padding: '10px', borderRadius: 12 }}
          >
            <Plus size={14} /> Nouveau QR
          </button>
        </div>
      )}
    </div>
...
function DemandsTab({ onCountChange }: { onCountChange: (n: number) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} className="fadein">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <button
          onClick={load}
          style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#9CA3AF', padding: '6px 10px', borderRadius: 8, fontWeight: 500 }}
        >
      ...
      {!loading && pending.length === 0 && (
        <div style={{ background: '#fff', borderRadius: 24, padding: '52px 24px', textAlign: 'center', boxShadow: '0 2px 16px rgba(21,16,31,.06)' }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 600, color: '#D1D5DB' }}>Aucune demande en attente</p>
        </div>
      )}

      {pending.map(d => (
        <div key={d.id} style={{ background: '#fff', borderRadius: 20, padding: 18, boxShadow: '0 2px 12px rgba(21,16,31,.07)', display: 'flex', alignItems: 'center', gap: 14 }} className="fadein">
          ...
          {acting === d.id ? (
            <Loader2 size={22} style={{ animation: 'spin .8s linear infinite', color: '#C4B5FD', flexShrink: 0 }} />
          ) : done[d.id] === 'accepted' ? (
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              <button onClick={() => act(d.id, 'reject')} style={{ width: 44, height: 44, borderRadius: 14, background: '#FEF2F2', color: '#EF4444', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <XCircle size={20} />
              </button>
              <button onClick={() => act(d.id, 'accept')} style={{ width: 44, height: 44, borderRadius: 14, background: '#F0FDF4', color: '#16A34A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {resolved.length > 0 && (
        <p style={{ fontSize: 12, color: '#D1D5DB', textAlign: 'center', marginTop: 4 }}>
...
function InstallBanner({ onInstall, onDismiss }: { onInstall: () => void; onDismiss: () => void }) {
  return (
    <div style={{ margin: '0 16px 0', background: '#EDE6FB', border: '1px solid #C4B5FD', borderRadius: 16, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }} className="fadein">
      ...
      <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
        <button onClick={onDismiss} style={{ fontSize: 12, color: '#9CA3AF', padding: '6px 10px', borderRadius: 8, fontWeight: 600 }}>Plus tard</button>
        ...
        <div style={{ background: '#15101F', padding: '14px 20px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', fontWeight: 600, marginTop: 2, letterSpacing: '0.06em' }}>{staff.name}</p>
            </div>
          </div>
          <button onClick={logout} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'rgba(255,255,255,.35)', padding: '8px 12px', borderRadius: 10, fontWeight: 600 }}>
